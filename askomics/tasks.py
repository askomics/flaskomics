...
def integrate(self, session, data, host_url):
    ...
    public = data.get("public", False) if session["user"]["admin"] else app.iniconfig.get("askomics", "single_tenant", fallback=False)
...
def query(self, session, info):
    ...
    try:
        ...
        results = []
        if info["graphs"] or app.iniconfig.get("askomics", "single_tenant", fallback=False):
